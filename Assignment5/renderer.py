...
class Renderer:
    ...
    def bling_phong(self, ray: Ray, rec: HitRecord):
        ...
        shadow_orig = (
            rec.pos + rec.N * eps
            if tm.dot(ray.rd, rec.N) < 0
            else rec.pos - rec.N * eps
        )
        ...
        for i in ti.static(range(len(self.lights.point_lights))):
            ...
            # diffuse
            diffuse_color += (
                tm.vec3(0) if in_shadow else max(0, tm.dot(light_dir, rec.N))
            )
            ...
            if frame.task_type == TASK_PROCESS:
                ...
                if rec.is_hit:
                    if m_type == REFLECTION_AND_REFRACTION or m_type == REFLECTION:
                        ...
                        reflect_dir += self.reflect_fuzz * random_in_unit_sphere()
                        ...
                        merge_frame = StatusFrame(
                            task_type=TASK_MERGE, depth=depth + 1, kr=kr
                        )
