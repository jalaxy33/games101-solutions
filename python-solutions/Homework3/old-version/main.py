...
class Rasterizer:
    def __init__(
        ...
        self.frame_buf = ti.Vector.field(
            3, float, shape=self.resolution
        )  # 屏幕像素颜色信息（rbg）
        self.depth_buf = ti.field(
            float, shape=self.resolution
        )  # 屏幕像素深度信息（z-buffer）
    ...
    def set_camera(self, camera: Camera):
        ...
        self.model[None] = get_model_matrix(
            self.data_angles, self.data_scales, self.data_translates
        )
    ...
    def bling_phong_fragment_shader(self, payload: FragmentShaderPayload) -> vec3:
        ...
        for i in ti.ndrange(self.lights.shape[0]):
            ...
            specular = (
                self.ks
                * E
                * tm.pow(tm.max(0, tm.dot(normal, half_dir)), self.phong_exp)
            )
    ...
    def displacement_fragment_shader(self, payload: FragmentShaderPayload) -> vec3:
        ...
        dU = self.kh * self.kn
        dU *= (
            self.texture.get_color(u + 1.0 / W, v).norm()
            - self.texture.get_color(u, v).norm()
        )
        dV = self.kh * self.kn
        dV *= (
            self.texture.get_color(u, v + 1.0 / H).norm()
            - self.texture.get_color(u, v).norm()
        )
        ln = vec3(-dU, -dV, 1)

        payload.color = self.texture.get_color(u, v)
        payload.viewspace_pos = (
            point + self.kn * n * self.texture.get_color(u, v).norm()
        )
        ...
        for x, y in ti.ndrange((x_min, x_max + 1), (y_min, y_max + 1)):
            ...
            for i, j in ti.ndrange(self.MSAA_N, self.MSAA_N):
                ...
                if inside_triangle(x_curr, y_curr, v1.xyz, v2.xyz, v3.xyz):
                    alpha, beta, gamma = compute_barycentric(
                        x_curr, y_curr, v1.xyz, v2.xyz, v3.xyz
                    )
                    w_reciprocal = 1.0 / (alpha / w1 + beta / w2 + gamma / w3)
                    z_interpolated = alpha * z1 / w1 + beta * z2 / w2 + gamma * z3 / w3
                    z_interpolated *= w_reciprocal
...
if __name__ == "__main__":
    # load model
    obj_file = os.path.abspath(
        "e:/projects/learn_something/games101/python-solutions/Homework3/models/spot/spot_triangulated.obj"
    )
    texture_png = os.path.abspath(
        "e:/projects/learn_something/games101/python-solutions/Homework3/models/spot/spot_texture.png"
    )
    ...
    # create camera
    camera = Camera(
        pos=(0, 0, 5), up=(0, 1, 0), look_at=(0, 0, -5), fov=60, zNear=0.1, zFar=50
    )
