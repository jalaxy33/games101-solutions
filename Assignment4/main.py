...
class Bezier:
    ...
    # 计算贝塞尔曲线第 t 个点的位置
    @ti.func
    def computeBezierPosition(self, t: int) -> vec2:
    ...
    def computeBezier(self):
        # TODO: [Task 1] Iterate through all t = 0 to t = 1 with small steps
        for t in range(self.t_num):
            point = self.computeBezierPosition(t)
            ...
            # # TODO: [Task 2] Anti-aliasing
            # for i, j in ti.ndrange((0, 2), (0, 2)):
            #     x_ = x + i
            #     y_ = y + j
            #     dist = tm.sqrt(tm.pow(x - x_, 2) + tm.pow(y - y_, 2))

            #     weight = 1 / (tm.exp(dist) + 1e-9)
            #     color = self.image_buf[x, y] * weight
            #     # print(i, j, dist, weight, color)
            #     self.image_buf[x_, y_] = color
